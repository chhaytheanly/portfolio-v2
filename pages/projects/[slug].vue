<template>
  ...
  >
    ...
    <header class="mb-10">
      <!-- Main Thumbnail -->
      <div class="aspect-video bg-[var(--color-bg-tertiary)] rounded-2xl overflow-hidden mb-6 border border-[var(--color-border)]/60 shadow-lg">
        <NuxtImg
        ...
        <div v-else class="w-full h-full bg-gradient-to-br from-[var(--color-bg-tertiary)] to-[var(--color-bg-secondary)] flex items-center justify-center">
          <svg class="w-24 h-24 text-[var(--color-text-tertiary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="flex flex-wrap items-center gap-4 mb-6 text-sm text-[var(--color-text-secondary)]">
        ...
        <div v-if="project.featured" class="px-3 py-1 rounded-full bg-[var(--color-accent)]/10 border border-[var(--color-accent)]/30">
      ...
      <div class="flex flex-wrap gap-3">
        <a
          ...
          class="btn-primary text-sm group inline-flex items-center gap-2"
          target="_blank"
          rel="noopener"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a
          ...
          class="btn-secondary text-sm group inline-flex items-center gap-2"
          target="_blank"
          rel="noopener"
        >
    ...
    <!-- Image Gallery -->
    <section v-if="project.images && project.images.length > 0" class="mb-10">
      <h2 class="text-xl font-bold mb-4">Project Gallery</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div
          v-for="(image, index) in project.images"
          :key="index"
          class="aspect-video rounded-xl overflow-hidden border border-[var(--color-border)]/60 cursor-pointer hover:scale-105 transition-transform duration-300"
    ...
    >
      <div class="relative max-w-5xl max-h-[90vh] w-full">
        ...
        />
        <button
          class="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center hover:bg-white/20 transition-colors"
          @click.stop="closeImageGallery"
        >
        ...
        <button
          ...
          class="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center hover:bg-white/20 transition-colors"
        ...
        <button
          v-if="project.images.length > 1"
          class="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center hover:bg-white/20 transition-colors"
        ...
        <!-- Image Counter -->
        <div class="absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full bg-black/50 backdrop-blur-md text-white text-sm">
...
const { data: project } = await useAsyncData<Project>(`project-${slug}`, async () => {
